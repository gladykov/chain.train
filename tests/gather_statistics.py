...
class GatherStatistics:
    ...
    def gather_statistics(self):
        query = "SELECT {distinct} {column_name} FROM {schema_name}.{table_name} {row_limiter}"

        gathered_statistics = []

        for table, column in self.schema.tables_columns_with_stats():
            for stat in column.gather_stats:
                distinct = "DISTINCT" if stat in [Stats.DISTINCT, Stats.DISTINCT_IN_RANGE] else ""
                ...
                if stat in [Stats.TOTAL_IN_RANGE, Stats.DISTINCT_IN_RANGE] and not table.row_limiter:
                    continue

                row_limiter = f"WHERE {table.row_limiter}" if table.row_limiter and stat in [Stats.TOTAL_IN_RANGE, Stats.DISTINCT_IN_RANGE] else ""
                value = self.db.count(self.db.query(query.format(
                    distinct=distinct,
